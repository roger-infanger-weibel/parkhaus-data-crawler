...
def get_weekly_log_stats():
    try:
        conn = db_utils.get_connection()
        cursor = conn.cursor(dictionary=True)
        # Group by Week, Parkhaus (first word), Severity
        # YEARWEEK(timestamp, 3) -> ISO 8601 week with Monday as first day
        query = """
            SELECT 
...
def get_stuck_parking():
    """
    Detect parking places with no changes in the last 24 hours.
    A parking place is considered 'stuck' if MIN(free) = MAX(free) for the day.
    """
    try:
        conn = db_utils.get_connection()
        cursor = conn.cursor(dictionary=True)
